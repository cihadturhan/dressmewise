...
!function(e) {
  ...
  function t(t) {
    ...
    "wheelDeltaX"in s && (r = -1 * s.wheelDeltaX),
    "axis"in s && s.axis === s.HORIZONTAL_AXIS && (r = -1 * d,
    d = 0),
    u = 0 === d ? r : d,
    "deltaY"in s && (d = -1 * s.deltaY,
    u = d),
    "deltaX"in s && (r = s.deltaX,
    0 === d && (u = -1 * r)),
    0 !== d || 0 !== r) {
      if (1 === s.deltaMode) {
      ...
      return f = Math.max(Math.abs(d), Math.abs(r)),
      (!l || l > f) && (l = f,
      i(s, f) && (l /= 40)),
      i(s, f) && (u /= 40,
      r /= 40,
...
function UGFunctions() {
  ...
  this.validateClickTouchstartEvent = function(eventType) {
    ...
    if (eventType == "click" && g_temp.lastEventType == "touchstart" && diff < 1E3)
      returnVal = false;
...
function UGLightbox() {
  ...
  function onItemChange(data, currentItem) {
    ...
    } else if (g_objSlider.isAnimating() == false) {
      if (g_objTextPanel)
        updateTextPanelText(currentItem);
      if (g_objNumbers)
        updateNumbersText(currentItem)
  ...
  this.open = function(index) {
    ...
    if (g_temp.isCompact == true) {
      ...
      else if (g_temp.isArrowsInside == true) {
        g_objArrowLeft.hide();
        g_objArrowRight.hide()
      }
    }
...
function UGZoomButtonsPanel() {
  ...
  function onZoomChange() {
    ...
    } else if (isButtonDisabled(g_objButtonMinus) == true) {
...
function UGStripPanel() {
  ...
  function placeElements_hor() {
    ...
    if (g_objButtonNext)
      stripX += g_objButtonNext.outerWidth() + g_options.strippanel_padding_buttons;
  ...
  function placeElements_vert() {
    ...
    if (g_objButtonNext)
      stripY += g_objButtonNext.outerHeight() + g_options.strippanel_padding_buttons;
...
function UGThumbsGrid() {
  ...
  function validateInnerPos(pos) {
    ...
    } else if (absPos < 0 || absPos >= g_temp.innerHeight) {
  ...
  function onTouchEnd(event) {
    ...
    if (direction == "next")
      if (g_options.grid_carousel == false && t.isLastPane())
        ...
        t.nextPane();
    else if (g_options.grid_carousel == false && t.isFirstPane())
      animateToCurrentPane();
    else
      t.prevPane()
  }
...
function UGTileDesign() {
  ...
  function setTextpanelEffect(objTile, isActive) {
    ...
    if (g_options.tile_textpanel_appear_type == "slide") {
      ...
      if (isActive == true) {
        ...
        objTextPanel.stop(true).animate({
          "bottom": endPos + "px"
        }, animationDuration)
      } else
        objTextPanel.stop(true).animate({
  ...
  function onTileClick(event) {
    ...
    if (tagname == "a") {
      if (isApproved == false)
        event.preventDefault()
    } else if (isOverStyle(objTile) == false) {
      if (isApproved == true)
  ...
  function handleSliderDrag(event) {
    ...
    if (objZoomSlider) {
      ...
      if (isPanEnabled == true) {
        ...
        return true
      } else if (g_temp.isInitDataValid == false) {
  ...
  function onTouchEnd(event) {
    ...
    if (numTouches == 0 && g_temp.touch_active == true) {
      ...
      if (isValid == true)
        ...
        returnToPlace()
    } else if (numTouches == 1 && g_temp.touch_active == false)
...
function UGZoomSliderControl() {
  ...
  function panImage(event) {
    ...
    if (g_temp.panYActive == false)
      diffLastY = 0;
    else if (directionY == "down" && posImage.top > 0)
      diffLastY = diffLastY / 3;
    else if (directionY == "up" && posImage.bottom < g_temp.objParentSize.height)
      diffLastY = diffLastY / 3;
    if (g_temp.panXActive == false || g_parent.isInnerInPlace() == false)
      diffLastX = 0;
    else if (directionX == "right" && posImage.left > 0)
  ...
  function checkReturnAfterPan() {
    ...
    if (g_temp.panYActive == true)
      if (objSize.top > 0) {
        ...
        if (objSize.bottom < g_temp.objParentSize.height) {
          newY = g_temp.objParentSize.height - objSize.height;
          isReturnY = true
        }
      }
    else if (objSize.top != objCenterPos.top) {
      isReturnY = true;
    ...
    if (g_temp.panXActive == true)
      if (objSize.left > 0) {
        ...
        if (objSize.right < g_temp.objParentSize.width) {
          newX = g_temp.objParentSize.width - objSize.width;
          isReturnX = true
        }
      }
    else if (objSize.left != objCenterPos.left) {
      isReturnX = true;
  ...
  this.isPanEnabled = function(event, direction) {
    ...
    if (direction == "left") {
      if (g_temp.objImageSize.right <= g_temp.objParentSize.width)
        return false
    } else if (g_temp.objImageSize.left >= 0)
      return false;
    return true
  }
  ;
